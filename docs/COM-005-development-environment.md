...
-   부모 Account 인증 — 이메일 + 비밀번호
-   **학생 인증 — 아이디 + 비밀번호** (2026-09-10 추가)

학생 계정은 **부모가 만들어 준다.** 아이는 스스로 가입하지 않는다.

아이에게 이메일 주소를 받지 않으므로, 서버가 아이디를 가짜 이메일
(`<login_id>@student.aipm.invalid`)로 바꿔 Auth에 넘긴다. 도메인은
`lib/constants/student-login.ts` 한 곳에 있다. **바꾸면 이미 만든 아이
계정이 전부 로그인하지 못한다.**

로그인 화면은 칸 하나다. `@`가 있으면 부모의 이메일, 없으면 아이의
아이디로 본다. 「보호자용」/「학생용」을 고르게 하지 않는다.

`auth.users` 행을 만드는 것은 Admin API(service_role)뿐이다. DEV-001 §8의
예외이며, 부모 세션으로 소유를 먼저 확인한 뒤에만 부른다
(`lib/services/student-login.ts`).
