...
- `lib/supabase/admin.ts`(service_role)는 **서버 전용**이다. `'use client'`
  파일에서 import 금지.
- service_role은 배치·웹훅 전용이다. 사용자 요청을 그대로 대신 수행하는
  용도로 쓰지 않는다.
...
2  본인 것인지 코드에서 다시 확인한다
3  그때만 service_role을 손에 쥔다
4  우리 테이블은 다시 사용자 세션으로 고친다
```

| 파일 | 하는 일 | service_role을 쓰는 범위 |
|---|---|---|
...
### 6-1. 디자인 트랙과 기능 트랙 (2026-09-10 추가)

한 화면을 두 사람이 만든다. **브랜치를 나눠도 합칠 때 부딪힌다** — 나뉘는
기준은 브랜치가 아니라 경로다(§6). `.github/CODEOWNERS`가 같은 표를 들고
있으며, 해당 경로를 건드리는 PR에 소유자가 자동으로 리뷰어로 붙는다.

| 경로 | 담당 |
|---|---|
| `src/app/**/page.tsx` · `layout.tsx` · `_components/**` | 디자인 |
| `src/components/**` · `globals.css` · `public/**` | 디자인 |
| `src/lib/**` · `src/app/api/**` · `**/_actions.ts` | 기능 |
| `supabase/migrations/**` · `src/types/**` · `docs/**` | 기능 |

`_actions.ts`는 화면과 같은 폴더에 있지만 **로직이다.** 화면이 무엇을 그릴지는
디자인이 정하고, 무엇을 저장할지는 기능이 정한다.

**화면은 이미 있는 `_actions.ts`를 그대로 부른다.** AI 연동을 새로 만들지
않는다. 2026-09-10에 디자인 브랜치가 `/api/ai/chat`과 `lib/ai/drilldown.ts`로
MODE A를 다시 구현해 같은 일을 하는 두 벌이 생겼다. 같은 테이블을 다른 함수
이름으로 읽고 썼다(`insertMessage` ↔ `appendMessage`).

브랜치가 오래되면 이 표를 지켜도 힘들어진다. **정기적으로 develop 위로
rebase한다.** 그 브랜치는 135 커밋 뒤에서 갈라져 있었다.
