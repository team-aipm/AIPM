...
`login_id`와 `auth_user_id`는 **등록할 때 반드시 받는다** (2026-09-17 개정).
아이는 자기 아이디로 자기 기기에서 들어간다.

전에는 둘 다 선택이었다 — 없으면 부모가 로그인한 기기에서 프로필을 골라
들어가는 길이 있었기 때문이다. **그 길을 닫았다**(COM-003 §4.2). 부모 계정은
학생 화면에 들어가지 않으므로, 아이디가 없으면 그 아이는 학습을 시작할 방법이
아예 없다.

**DB 는 nullable 로 둔다.** 이 규칙 이전에 아이디 없이 등록된 학생이 있어서
`NOT NULL` 로 조이면 그 행이 깨진다. 막는 자리는 등록 화면과 서버(`STU-001` ·
`MY-004`)다. 남아 있는 학생은 `MY-003`에서 아이디를 만들어 준다 — 그 전까지
그 아이는 들어오지 못한다.
