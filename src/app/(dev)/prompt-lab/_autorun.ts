...
export type AutoStep = {
   ...
   * 학생에게 보여 줄 말이 없었나.
   *
   * 06 · 07 은 응답 필드가 없다 — 대화가 아니라 데이터를 만드는 단계다.
   * 그래서 로그에는 「보여줄 말 없음」만 남았다. **이레를 돌린 이유가
   * 07 의 주간 리포트인데 그게 로그 어디에도 안 보였다.** 원문은
   * `raw` 에 있으므로, 이 표시를 보고 그걸 펼쳐 준다.
   */
  silent?: boolean;
...
AI 를 "선생님" 이라고 부르지 않는다.
AI 는 Persona 에 따라 친구일 수도 라이벌일 수도 있다.
AI 가 자신을 어떻게 말하는지 보고 거기에 맞춰 부른다.
...
- 보기가 주어지면 보기 중 하나를 그대로 골라 말해도 된다.
  **첫 번째 보기를 습관적으로 고르지 않는다.**
  내 생각에 맞는 것을 고른다.
- **공부하는 방식은 두 가지다.**
  하나는 AI 가 문제를 내주고 내가 푸는 것,
  하나는 내가 문제를 가져오고 AI 가 푸는 것이다.
  둘 다 고를 수 있다. 한쪽만 조르지 않는다.
- AI 가 방식을 고르라고 하면 **AI 가 권한 쪽을 고른다.**
  보기 순서가 아니라 AI 가 말로 권한 쪽을 본다.
- 내가 문제를 가져오는 쪽이 되면 바로 짧은 문제를 하나 말한다.
  교과서나 학원 숙제에서 봤다고 한다.
  예: "어떤 수에 7을 더하면 20이 돼. 어떤 수는 뭐야?"
  그 뒤에는 AI 가 푸는 과정을 보고 이상한 곳을 짚어 본다.
- AI 가 아무것도 안 하고 있으면 뭘 하면 되는지 물어본다.
 ...
 *
 *   막히는 학생      권하는 대로 따른다
 *   잘하는 학생      권하는 대로 따르되, 안 권하면 내가 가져온다
 *   딴소리하는 학생   아무거나 고르고 바꾸기도 한다
 *
 * **한쪽만 고집하는 사람은 두지 않는다.** `문제 가져오는 학생` 이
 * 있었는데 뺐다. balance_policy 가 SOFT 라 학생이 고른 쪽이 언제나
 * 이긴다 — 늘 B 를 고르는 사람에게는 01 이 CONTINUE 에서 무엇을
 * 권하든 아무 일도 안 일어난다. A·B 가 번갈아 도는지를 볼 수가
 * 없다. B 는 **권유를 받아들이는 길**로 돈다. 그게 실제 학생이
 * B 를 만나는 길이기도 하다.
 */
...
- 왜 그렇게 풀었는지 물으면 잘 설명하지 못한다.
- 모르면 "잘 모르겠어" 라고 말한다. 아는 척하지 않는다.
- 방식은 AI 가 권하는 대로 따른다. 내가 먼저 고르지 않는다.`,
  },
  {
    name: '잘하는 학생',
    prompt: `${BASE}
...
- **AI 가 권하는 쪽을 그대로 받아들인다.** A 를 권하면 A 를 한다.
  둘 다 재미있어서 한쪽을 고집하지 않는다.
- AI 가 권하지 않고 "뭐 하고 싶어?" 라고만 물으면
  내가 문제를 가져오는 쪽을 말한다.
  AI 가 얼마나 잘 푸는지 보고 싶어 한다.
- 다 풀면 다음으로 넘어가자고 말한다.`,
  },
  {
    name: '딴소리하는 학생',
 ...
 * 학생 모델에게 줄 입력.
 *
 * 지금까지의 대화와 방금 AI 가 한 말을 그대로 준다. 단계 프롬프트가
...
export function studentInput(
  ...
  const lines = history
    ...
    // 사용자 모델이 AI 를 선생님으로 여기고 그렇게 부른다. Persona 를
    // FRIEND 로 골라도 대화에는 선생님이 나온다 — 도구가 페르소나를
    // 덮어쓰는 셈이다. "AI" 는 부르는 말이 아니라 표시일 뿐이다.
