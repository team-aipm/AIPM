...
- `docs/COM-004` (본문 완료 · 검토 대기) — 코딩 규칙 · 리뷰 기준 · 테스트 기준
- `docs/COM-007` (본문 완료 · 검토 대기) — 회원탈퇴 처리, 운영자 데이터 열람 범위
- `docs/COM-005 §13` (미확정) — **결제 PG.** BIL 영역 전체가 여기서 막혀 있다
- ADM 화면을 새로 만들 때는 `COM-003 §4.8` 에 먼저 올린다
- `src/app/(admin)/`은 COM-007 §7 · COM-002 §20-B 근거로 존재한다. 운영자 추가는 SQL 로만
